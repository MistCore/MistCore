    ...
    struct instance_onyxias_lair_InstanceMapScript : public InstanceScript
    {
        ...
        void FloorEruption(uint64 floorEruptedGUID)
        {
            if (GameObject* pFloorEruption = instance->GetGameObject(floorEruptedGUID))
            {
                ...
                MistCore::GameObjectInRangeCheck check(pFloorEruption->GetPositionX(), pFloorEruption->GetPositionY(), pFloorEruption->GetPositionZ(), 15);
                MistCore::GameObjectListSearcher<MistCore::GameObjectInRangeCheck> searcher(pFloorEruption, nearFloorList, check);

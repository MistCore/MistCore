    ...
    public:
        ...
        class spell_hun_glaive_toss_damage_SpellScript : public SpellScript
        {
            ...
            void CorrectDamageRange(std::list<WorldObject*>& targets)
            {
                ...
                MistCore::NearestAttackableUnitInObjectRangeCheck u_check(GetCaster(), GetCaster(), radius);
                MistCore::UnitListSearcher<MistCore::NearestAttackableUnitInObjectRangeCheck> searcher(GetCaster(), targetList, u_check);
        ...
        class spell_hun_binding_shot_zone_AuraScript : public AuraScript
        {
            ...
            void OnUpdate(uint32 diff, AuraEffectPtr aurEff)
            {
                if (Unit* caster = GetCaster())
                {
                    ...
                    CellCoord p(MistCore::ComputeCellCoord(dynObj->GetPositionX(), dynObj->GetPositionY()));
                    Cell cell(p);
                    cell.SetNoCreate();

                    MistCore::AnyUnitInObjectRangeCheck u_check(dynObj, 15.0f);
                    MistCore::UnitListSearcher<MistCore::AnyUnitInObjectRangeCheck> searcher(dynObj, bindedList, u_check);
                    ...
                    bindedList.remove_if(MistCore::UnitAuraCheck(false, GetSpellInfo()->Id, caster->GetGUID()));
...
class spell_hun_ancient_hysteria : public SpellScriptLoader
{
    public:
        ...
        class spell_hun_ancient_hysteria_SpellScript : public SpellScript
        {
            ...
            {
                targets.remove_if(MistCore::UnitAuraCheck(true, HUNTER_SPELL_INSANITY));
                targets.remove_if(MistCore::UnitAuraCheck(true, SPELL_SHAMAN_EXHAUSTED));
                targets.remove_if(MistCore::UnitAuraCheck(true, SPELL_SHAMAN_SATED));
                targets.remove_if(MistCore::UnitAuraCheck(true, SPELL_MAGE_TEMPORAL_DISPLACEMENT));
            }
    ...
    public:
        ...
        class spell_hun_pet_carrion_feeder_SpellScript : public SpellScript
        {
            ...
            SpellCastResult CheckIfCorpseNear()
            {
                ...
                MistCore::AnyDeadUnitSpellTargetInRangeCheck check(caster, max_range, GetSpellInfo(), TARGET_CHECK_ENEMY);
                MistCore::WorldObjectSearcher<MistCore::AnyDeadUnitSpellTargetInRangeCheck> searcher(caster, result, check);

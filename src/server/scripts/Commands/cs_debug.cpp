...
public:
    ...
    static bool HandleDebugEnterVehicleCommand(ChatHandler* handler, char const* args)
    {
        ...
        else
        {
            ...
            MistCore::AllCreaturesOfEntryInRange check(handler->GetSession()->GetPlayer(), entry, 20.0f);
            MistCore::CreatureSearcher<MistCore::AllCreaturesOfEntryInRange> searcher(handler->GetSession()->GetPlayer(), passenger, check);
